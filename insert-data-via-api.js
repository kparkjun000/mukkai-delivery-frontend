// Node.js script to insert all mock data to Railway backend
const axios = require('axios');

const API_URL = 'https://web-production-dc795.up.railway.app';
...
async function insertMenus(storeIds) {
  console.log('\nStarting to insert menus...');
  
  for (let i = 0; i < storeIds.length; i++) {
    const storeId = storeIds[i];
    const storeMenus = menus[i + 1];
    
    if (!storeMenus) continue;
    
    for (const menu of storeMenus) {
      try {
        await axios.post(`${API_URL}/api/store-menu/register`, {
          storeId: storeId,
          ...menu
        }, {
          headers: { 'Content-Type': 'application/json' }
        });
        console.log(`✓ Inserted menu: ${menu.name} for store ID ${storeId}`);
      } catch (error) {
        console.error(`✗ Failed to insert menu ${menu.name}:`, error.response?.data || error.message);
      }
    }
  }
}
...
async function main() {
  ...
  try {
    ...
    if (!healthCheck) {
      console.log('Warning: Backend might not be fully accessible. Proceeding anyway...\n');
    }
    
    const storeIds = await insertStores();
    await insertMenus(storeIds);
