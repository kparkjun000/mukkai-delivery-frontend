...
app.get('/assets/index-BUhxMOPx.js', (req, res) => {
  ...
  const newJsPath = path.join(__dirname, 'dist', 'assets', 'index-DnBxKYNh.js');
...
app.get('/assets/index-BUhxMOPx.*', (req, res) => {
  ...
  const newUrl = req.url.replace('index-BUhxMOPx', 'index-DnBxKYNh');
  res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
...
<html lang="en">
  <head>
    ...
    <script type="module" crossorigin src="/assets/index-DnBxKYNh.js?v=${timestamp}&bust=${randomId}&t=${Date.now()}"></script>
